...
def generate_row(client, row, sys_prompt, bul_prompt, use_bullets,
    ...
    result["_DURATION_S"]       = item_duration
    return result
...
if run_clicked and can_run:
    ...
    avg_dur    = sum(r.get("_DURATION_S", 0) for r in results) / len(results)
    ...
    m6.metric("Avg duration", f"{avg_dur:.1f}s")

    for r in results:
        label     = r.get("ITEM_TITLE") or r.get("NATURAL_KEY") or list(r.values())[0]
        item_cost = r.get("_COST_USD", 0)
        item_dur  = r.get("_DURATION_S", 0)
        with st.expander(f"**{label}** — {r['CHAR_COUNT']} chars · ${item_cost:.4f} · {item_dur:.1f}s"):
